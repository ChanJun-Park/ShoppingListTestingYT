...
class ShoppingViewModel @Inject constructor(
    ...
    fun insertShoppingItem(name: String, amountString: String, priceString: String) {
        ...
        if (name.length > Constants.MAX_NAME_LENGTH) {
            _insertShoppingItemStatus.postValue(Event(Resource.error("The name of item must not exceed ${Constants.MAX_NAME_LENGTH} charaterts", null)))
            return
        }

        if (priceString.length > Constants.MAX_PRICE_LENGTH) {
            _insertShoppingItemStatus.postValue(Event(Resource.error("The price of item must not exceed ${Constants.MAX_PRICE_LENGTH} charaterts", null)))
            return
        }
